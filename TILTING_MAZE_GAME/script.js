...
// Joystick start event
joystickHeadElement.addEventListener("mousedown", function (event) {
  if (!gameInProgress && startScreen.classList.contains("hidden")) {
    startGameFromJoystick(event);
...
joystickHeadElement.addEventListener("touchstart", function (event) {
  ...
  if (!gameInProgress && startScreen.classList.contains("hidden")) {
    ...
    startGameFromJoystick({ clientX: touch.clientX, clientY: touch.clientY });
  } else if (gameInProgress) {
    const touch = event.touches[0];
    handleJoystickStart({ clientX: touch.clientX, clientY: touch.clientY });
  }
});
...
function startGameFromJoystick(event) {
  mouseStartX = event.clientX;
  mouseStartY = event.clientY;
  gameInProgress = true;
  gameStatus.textContent = "Playing";
  joystickHeadElement.classList.add("active");
...
function updateModeIndicator() {
  if (hardMode) {
    modeIndicator.classList.add("hard");
    modeText.textContent = "Hard Mode";
  } else {
    modeIndicator.classList.remove("hard");
...
window.addEventListener("mouseup", function () {
  joystickHeadElement.classList.remove("active");
});
...
window.addEventListener("keydown", function (event) {
  ...
  if (event.key == "H" || event.key == "h") {
    hardMode = true;
    difficultyBtns.forEach((b) => b.classList.remove("active"));
    document.querySelector('[data-mode="hard"]').classList.add("active");
    updateModeIndicator();
    resetGame();
    return;
  }
...
function main(timestamp) {
  ...
  try {
    ...
    if (error.message == "The ball fell into a hole") {
      ...
      // Show failure message - briefly show modal or just reset
      setTimeout(() => {
        alert("A ball fell into a black hole! Press Space to restart.");
        resetGame();
      }, 100);
    } else throw error;
  }
